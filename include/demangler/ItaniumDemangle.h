...
class PODSmallVector {
    static_assert(std::is_pod<T>::value, "T is required to be a plain old data type");
...
public:
    ...
    : Node(KQualType, Child_->RHSComponentCache, Child_->ArrayCache, Child_->FunctionCache),
      Quals(Quals_),
      Child(Child_) {}
...
struct AbiTagAttr : Node {
    ...
    : Node(KAbiTagAttr, Base_->RHSComponentCache, Base_->ArrayCache, Base_->FunctionCache),
      Base(Base_),
      Tag(Tag_) {}
...
public:
    ...
    std::string_view getBaseName() const override {
        ...
        if (isInstantiation()) {
            ...
            assert(demangler::itanium_demangle::starts_with(SV, "basic_"));
...
public:
    ClosureTypeName(NodeArray TemplateParams_, NodeArray Params_, std::string_view Count_)
...
struct AbstractManglingParser {
    ...
    bool consumeIf(std::string_view S) {
        if (demangler::itanium_demangle::starts_with(std::string_view(First, Last - First), S)) {
            First += S.size();
            return true;
        }
        return false;
    ...
    public:
        std::string_view getSymbol() const {
            ...
            if (Kind < Unnameable) {
                assert(
                    demangler::itanium_demangle::starts_with(Res, "operator")
                    && "operator name does not start with 'operator'"
                );
                Res.remove_prefix(sizeof("operator") - 1);
                if (demangler::itanium_demangle::starts_with(Res, ' ')) Res.remove_prefix(1);
            }
            return Res;
        }
        std::string_view getName() const { return Name; }
...
Node* AbstractManglingParser<Derived, Alloc>::parseUnqualifiedName(NameState* State, Node* Scope, ModuleName* Module) {
    ...
    if (Result != nullptr && Scope != nullptr) Result = make<NestedName>(Scope, Result);
...
Node* AbstractManglingParser<Derived, Alloc>::parseUnnamedTypeName(NameState* State) {
    ...
    if (consumeIf("Ul")) {
        ...
        while (look() == 'T' && std::string_view("yptn").find(look(1)) != std::string_view::npos) {
...
Node* AbstractManglingParser<Derived, Alloc>::parseSourceName(NameState*) {
    ...
    First += Length;
    if (demangler::itanium_demangle::starts_with(Name, "_GLOBAL__N")) return make<NameType>("(anonymous namespace)");
...
Node* AbstractManglingParser<Derived, Alloc>::parseQualifiedType() {
    if (consumeIf('U')) {
        ...
        if (demangler::itanium_demangle::starts_with(Qual, "objcproto")) {
