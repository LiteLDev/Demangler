...
private:
    ...
    void parseLName(OutputBuffer* Demangled, std::string_view& Mangled, unsigned long Len);
...
bool Demangler::decodeBackrefPos(std::string_view& Mangled, long& Ret) {
    ...
    while (!Mangled.empty() && std::isalpha(Mangled.front())) {
        // Check for overflow
        if (Val > (std::numeric_limits<unsigned long>::max() - 25) / 26) break;
...
bool Demangler::decodeBackref(std::string_view& Mangled, std::string_view& Ret) {
    assert(!Mangled.empty() && Mangled.front() == 'Q' && "Invalid back reference!");
    Ret = {};
...
void Demangler::parseSymbolBackref(OutputBuffer* Demangled, std::string_view& Mangled) {
...
bool Demangler::isSymbolName(std::string_view Mangled) {
    ...
    if (std::isdigit(Mangled.front())) return true;

    // TODO: Handle template instances.

    if (Mangled.front() != 'Q') return false;

    Mangled.remove_prefix(1);
    bool Valid = decodeBackrefPos(Mangled, Ret);
    if (!Valid || Ret > Qref - Str.data()) return false;
...
void Demangler::parseMangle(OutputBuffer* Demangled, std::string_view& Mangled) {
    ...
    if (Mangled.front() == 'Z') {
        Mangled.remove_prefix(1);
    } else if (!parseType(Mangled)) Mangled = {};
}

void Demangler::parseQualified(OutputBuffer* Demangled, std::string_view& Mangled) {
    ...
    do {
        ...
        if (!Mangled.empty() && Mangled.front() == '0') {
            do Mangled.remove_prefix(1);
            while (!Mangled.empty() && Mangled.front() == '0');

            continue;
        }

        if (NotFirst) *Demangled << '.';
        NotFirst = true;
...
void Demangler::parseIdentifier(OutputBuffer* Demangled, std::string_view& Mangled) {
    if (Mangled.empty()) {
        Mangled = {};
        return;
    }

    if (Mangled.front() == 'Q') return parseSymbolBackref(Demangled, Mangled);
    ...
    if (Len >= 4 && starts_with(Mangled, "__S")) {
        ...
        while (P.length() > SuffixLen && std::isdigit(P.front())) P.remove_prefix(1);
...
void Demangler::parseLName(OutputBuffer* Demangled, std::string_view& Mangled, unsigned long Len) {
    switch (Len) {
    case 6:
...
Demangler::Demangler(std::string_view Mangled) : Str(Mangled), LastBackref(Mangled.length()) {}
...
char* demangler::dlangDemangle(std::string_view MangledName) {
    if (MangledName.empty() || !starts_with(MangledName, "_D")) return nullptr;
