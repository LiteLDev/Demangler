...
using demangler::itanium_demangle::ScopedOverride;
...
private:
    bool demanglePath(IsInType Type, LeaveGenericsOpen LeaveOpen = LeaveGenericsOpen::No);
...
char* demangler::rustDemangle(std::string_view MangledName) {
    // Return early if mangled name doesn't look like a Rust symbol.
    if (MangledName.empty() || !starts_with(MangledName, "_R")) return nullptr;
...
Demangler::Demangler(size_t MaxRecursionLevel) : MaxRecursionLevel(MaxRecursionLevel) {}

static inline bool isDigit(const char C) { return '0' <= C && C <= '9'; }

static inline bool isHexDigit(const char C) { return ('0' <= C && C <= '9') || ('a' <= C && C <= 'f'); }
...
static inline bool isValid(const char C) { return isDigit(C) || isLower(C) || isUpper(C) || C == '_'; }
...
bool Demangler::demangle(std::string_view Mangled) {
    ...
    if (Position != Input.size()) Error = true;
...
bool Demangler::demanglePath(IsInType InType, LeaveGenericsOpen LeaveOpen) {
    ...
    case 'N': {
        ...
        if (isUpper(NS)) {
            // Special namespaces
            print("::{");
            if (NS == 'C') print("closure");
            else if (NS == 'S') print("shim");
            else print(NS);
            if (!Ident.empty()) {
                print(":");
    ...
    case 'I': {
        ...
        if (InType == IsInType::No) print("::");
        print("<");
        for (size_t I = 0; !Error && !consumeIf('E'); ++I) {
            if (I > 0) print(", ");
            demangleGenericArg();
        }
        if (LeaveOpen == LeaveGenericsOpen::Yes) return true;
        else print(">");
        break;
    }
    case 'B': {
...
void Demangler::demangleGenericArg() {
    if (consumeIf('L')) printLifetime(parseBase62Number());
    else if (consumeIf('K')) demangleConst();
    else demangleType();
}
...
void Demangler::demangleType() {
    ...
    char      C     = consume();
    BasicType Type;
    if (parseBasicType(C, Type)) return printBasicType(Type);
...
void Demangler::demangleConstBool() {
    ...
    if (HexDigits == "0") print("false");
    else if (HexDigits == "1") print("true");
    else Error = true;
}

/// Returns true if CodePoint represents a printable ASCII character.
static bool isAsciiPrintable(uint64_t CodePoint) { return 0x20 <= CodePoint && CodePoint <= 0x7e; }
...
uint64_t Demangler::parseOptionalBase62Number(char Tag) {
    ...
    if (Error || !addAssign(N, 1)) return 0;
...
uint64_t Demangler::parseHexNumber(std::string_view& HexDigits) {
    ...
    if (!isHexDigit(look())) Error = true;

    if (consumeIf('0')) {
        if (!consumeIf('_')) Error = true;
    } else {
...
static bool decodePunycode(std::string_view Input, OutputBuffer& Output) {
    ...
    for (size_t I = 0; InputIdx != Input.size(); I += 1) {
        ...
        for (size_t K = Base; true; K += Base) {
            ...
            char   C     = Input[InputIdx++];
            size_t Digit = 0;
            if (!decodePunycodeDigit(C, Digit)) return false;

            if (Digit > (Max - I) / W) return false;
            I += Digit * W;
...
void Demangler::printIdentifier(Identifier Ident) {
    ...
    if (Ident.Punycode) {
        if (!decodePunycode(Ident.Name, Output)) Error = true;
    } else {
        print(Ident.Name);
    }
}
...
char Demangler::look() const {
    if (Error || Position >= Input.size()) return 0;
...
bool Demangler::consumeIf(char Prefix) {
    if (Error || Position >= Input.size() || Input[Position] != Prefix) return false;
