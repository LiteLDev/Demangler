...
using namespace demangler::itanium_demangle;
...
struct DumpVisitor {
    ...
    void print(std::string_view SV) { fprintf(stderr, "\"%.*s\"", (int)SV.size(), SV.data()); }
    void print(const Node* N) {
        if (N) N->visit(std::ref(*this));
        else printStr("<null>");
    }
    ...
    template <class T>
    std::enable_if_t<std::is_unsigned<T>::value> print(T N) {
        fprintf(stderr, "%llu", (unsigned long long)N);
    }

    template <class T>
    std::enable_if_t<std::is_signed<T>::value> print(T N) {
...
static char* printNode(const Node* RootNode, char* Buf, size_t* N) {
    ...
    RootNode->print(OB);
    OB += '\0';
    if (N != nullptr) *N = OB.getCurrentPosition();
    return OB.getBuffer();
}
...
KeepGoingLocalFunction:
    ...
    if (Name->getKind() == Node::KModuleEntity) Name = static_cast<const ModuleEntity*>(Name)->Name;
...
char* ItaniumPartialDemangler::getFunctionReturnType(char* Buf, size_t* N) const {
    ...
    if (const Node* Ret = static_cast<const FunctionEncoding*>(RootNode)->getReturnType()) Ret->print(OB);
